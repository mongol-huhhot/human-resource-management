    ...
    const fields = Array.isArray(rows)
      ? rows
          ...
          .map(row => {
            ...
            return {
              label: def.label || row.item_description?.label || row.l_item_code || row.g_item_code,
  ...
  <v-card variant="outlined" class="pa-4">
          ...
          <v-row dense>
            <v-col cols="12" md="6">
              <v-select
                ...
                variant="outlined"
                density="comfortable"
                :loading="loadingUiItems"
                :disabled="!model.app_code || !model.process_code"
    ...
    <v-card-actions class="justify-end">
      <v-btn
        color="primary"
        :loading="saving"
        :disabled="!props.tabConfig?.sqltags?.save"
        @click="save"
      >
        保存
      </v-btn>
